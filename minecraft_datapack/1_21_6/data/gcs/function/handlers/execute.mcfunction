...
execute unless data storage gcs:temp current_handler run tellraw @s [{"text":"[!] ","color":"red","bold":true},{"text":"Handler not found!","color":"red"}]
execute unless data storage gcs:temp current_handler run return fail

execute unless data storage gcs:temp current_handler{enabled:1b} run tellraw @s [{"text":"[!] ","color":"red","bold":true},{"text":"This handler is currently inactive!","color":"red"}]
...
execute if score @s gcs.auth < #required_auth gcs.count run tellraw @s [{"text":"[!] ","color":"red","bold":true},{"text":"Auth Levelniz yetersiz!","color":"red"}]
execute if score @s gcs.auth < #required_auth gcs.count run return fail

$function gcs:exec/_record_recent {hid:$(hid)}

# Multi-command handler — redirect to separate function (exit with return)
execute if data storage gcs:temp current_handler.commands run return run function gcs:handlers/execute_multi

# Tek commandlu handler
