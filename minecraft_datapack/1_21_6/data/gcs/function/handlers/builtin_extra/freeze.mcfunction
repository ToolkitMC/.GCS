...
# OFF -> ON (freeze)
$execute if score #fz_state gcs.count matches 0 as $(target) run tag @s add gcs_frozen
...
# ON -> OFF (unfreeze)
$execute if score #fz_state gcs.count matches 1 as $(target) run tag @s remove gcs_frozen
$execute if score #fz_state gcs.count matches 1 as $(target) run gamemode survival @s
