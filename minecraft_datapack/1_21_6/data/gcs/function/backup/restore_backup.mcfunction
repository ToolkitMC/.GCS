...
execute unless data storage gcs:backup {has_snapshot:1b} run tellraw @s [{"text":"[✗] ","color":"red","bold":true},{"text":"Nothing to undo!","color":"red"}]
...
# Clear snapshot (single use)
data remove storage gcs:backup has_snapshot

tellraw @s [{"text":"[✓] ","color":"green","bold":true},{"text":"Undone!","color":"green"}]
