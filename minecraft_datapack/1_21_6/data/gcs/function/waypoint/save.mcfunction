# Usage: /function gcs:waypoint/save {name:"evim"}
execute unless score @s gcs.auth matches 1.. run return fail
# Get current length — new element will be at this index
...
# Update coordinates (positive index = current length)
...
$tellraw @s [{"text":"[📍] ","color":"light_purple","bold":true},{"text":"Waypoint saved: ","color":"white"},{"text":"$(name)","color":"yellow"}]
