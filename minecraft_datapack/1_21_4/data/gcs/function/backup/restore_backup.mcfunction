# Restore from backup
$execute unless data storage gcs:backup/$(timestamp) handlers run tellraw @s [{"text":"[✗] ","color":"red","bold":true},{"text":"Yedek bulunamadı!","color":"red"}]
$execute unless data storage gcs:backup/$(timestamp) handlers run return fail
